...
exports.handler = async (event, context, callback) => {
  if (req.method === "POST") {
    ...
    let { pageUrl, userId } = JSON.parse(event.body);
    if (pageUrl && userId) {
      ...
      if (!existingUser.empty) {
        ...
        const response = await fetch(
          "https://web-mark.netlify.app/.netlify/functions/makeScreenshot",
          {
            method: "post",
        ...
        let color;
        const { imgBuffer, pageTitle } = data;
        // detect vibrant light color
        Vibrant.from(imgBuffer).getPalette((error, palette) => {
        ...
        await cloudinary.v2.uploader.upload(
          ...
          (error, result) => {
            if (error) {
              throw new Error("CLOUDINARY UPLOAD ERRROR");
            }
